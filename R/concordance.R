...
function (db, test = "Default", B = 1000, alpha = 0.05) 
    {
        ...
        s.boot<-c()
        pi.v.boot<-replicate(B,pi.boot<-sample(pi,size=nrow(db),replace=TRUE))
        ...
        MC <- function(db) {
            ...
            matrix.mc<-replicate(B,matrix.rep<-matrix(,nrow=nrow(db),ncol=ncol(db)))
            pij.mc<-replicate(B,pij.rep<-matrix(,nrow=nrow(db),ncol=ncol(db)))								 
            ...
            for (k in 1:B) {
                for (i in 1:nrow(db)) {
                    for (j in 1:ncol(db)) {
                        pij.mc[i,j,k]<-matrix.mc[i,j,k]*(matrix.mc[i,j,k]-1)/(rowSums(db)[i] * 
                                                                                  (rowSums(db)[i] - 1))
                    }
                }
            }
        ...
        if (length(unique(rowSums(db))) == 1) {
            std.kappa <- sqrt(2/(nrow(db) * (rowSums(db)[1] * (rowSums(db)[1] - 
                                                                   1))))
            ...
            Chisq <- function(db) {
                ...
                pvalue <- 1 - pchisq(stat.test, df = (nrow(db) * 
                                                          (ncol(db) - 1)))
                kappa.vet<-c(fleiss.kappa,ci.low,ci.upp,std.kappa,z.fleiss,pvalue.fleiss)
                names(kappa.vet)<-c("Kappa","LCL","UCL","Std.Error","Z value", "Pr(>|z|)")
                s.vet<-c(s,s.boot.ci,pvalue)
                names(s.vet)<-c("S","LCL","UCL","pvalue")
            ...
            MC2 <- function(db) {
                matrix.mc<-replicate(B,matrix.rep<-t(rmultinom(nrow(db), size = rowSums(db)[1], 
                                                               prob = rep(1/(ncol(db)), ncol(db)))))
                pij.mc<-replicate(B,pij.rep<-matrix(,nrow=nrow(db),ncol=ncol(db)))								 
